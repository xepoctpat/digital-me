...
if ($BackendOnly) {
    ...
    & (Join-Path $scriptRoot 'stop.ps1')
    & (Join-Path $scriptRoot 'start.ps1') -UseOllama:$UseOllama -LocalLLMUrl $effectiveLocalLLMUrl -HealthTimeoutSec $HealthTimeoutSec -Force:$Force
}
