...
const NetWorkMemberList = (): JSX.Element => {
  ...
  const renderUploadList = () => {
    return uploads.map((upload) => {
      const uploadStatus =
        loadInfo !== null && upload.instance_id === loadInfo.instance_id
          ? loadInfo.status
          : upload.status;
      ...
      return (
        <div
          ...
          onClick={() => {
            window.open(
              getPublicPortalUrl(upload.upload_name, upload.instance_id),
              '_blank',
              'noopener,noreferrer'
            );
          }}
              ...
              <div className="flex items-center gap-2 mt-1">
                <div
                  className={`px-2 py-0.5 text-xs rounded-full flex items-center gap-1.5 ${
                    uploadStatus === 'offline' ? 'bg-gray-100 text-gray-800' : uploadStatus === 'registered' ? 'bg-yellow-100 text-yellow-800' : 'bg-green-100 text-green-800'
                  }`}
                >
  ...
  if (!PUBLIC_NETWORK_ENABLED) {
    return (
      <div className="text-gray-500 text-sm p-8 bg-gray-50 rounded-lg border border-gray-200 text-center shadow-sm">
        ...
        <p className="text-gray-600 font-medium">Private Mode</p>
        <p className="text-sm text-gray-400 mt-2">{PRIVATE_MODE_MESSAGE}</p>
      </div>
    );
  }
